...
import { runSnapshotSimulation, type VisualizationData } from "./simulation/snapshotEngine";
...
function App() {
  ...
  const handleWatchLive = useCallback(
    (scenario: WillingnessScenario, startMonth: string, platformAdoptionRate: number) => {
      const config = {
      ...
      const seed = Date.now();
      const dataSet = MATCHING_ALGORITHMS.map((algo) =>
        runSnapshotSimulation(config, algo, seed)
      );
      setOutput(null);
      setVizData(dataSet);
    },
    ...
    <div className="min-h-screen font-sans">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        {vizData ? (
          ...
          <>
            <SimulationSetup onRun={handleRun} onWatchLive={handleWatchLive} isRunning={isRunning} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={s.n} className={`border-l-2 ${s.accent} pl-4 py-1`}>
                  ...
                  <h4 className="text-sm font-semibold text-zinc-200 mt-1">{s.title}</h4>
