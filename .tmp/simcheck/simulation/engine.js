...
function getMonthForDay(_day, startMonthIndex) {
    return MONTH_NAMES[startMonthIndex % 12];
}
// MAIN SIMULATION FUNCTION
function runSimulation(config, algorithm, seed, runId) {
    const rng = new distributions_1.SeededRNG(seed);
    const dispatchRng = new distributions_1.SeededRNG(seed + 1);
...
// COMPUTE SUMMARY METRICS
function computeSummary(algorithm, allRequests, allTravellers, dailyMetrics) {
    ...
    const avgDeliveryTime = fulfilledRequests.length > 0
    ...
    const urgentFulfillmentRate = urgentRequests.length > 0 ? urgentFulfilled / urgentRequests.length : 0;
    // Total weight delivered
    const totalWeightDelivered = fulfilledRequests.reduce((sum, r) => sum + r.weightKg, 0);
    // Average backlog size
    const avgBacklog = dailyMetrics.reduce((sum, d) => sum + d.backlogSize, 0) / dailyMetrics.length;
    ...
    const matchedTravellers = allTravellers.filter((t) => t.assignedRequestIds.length > 0);
    const avgCapacityUtilisation = matchedTravellers.length > 0
        ? matchedTravellers.reduce((sum, t) => sum + t.usedCapacityKg / t.spareCapacityKg, 0) / matchedTravellers.length
        : 0;
    // Expired requests
    ...
    for (const country of constants_1.COUNTRIES) {
        ...
        const countryAvgDelivery = countryFulfilled.length > 0
            ? countryFulfilled
        ...
        byCountry[country] = {
            ...
            fulfillmentRate: countryRequests.length > 0
