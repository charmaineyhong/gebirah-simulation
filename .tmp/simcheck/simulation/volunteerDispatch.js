...
exports.dispatchVolunteers = dispatchVolunteers;
const distributions_1 = require("./distributions");
function dispatchVolunteers(rng, requests, travellers, volunteers, currentDay) {
    const result = {
    ...
    const departingTravellers = travellers.filter((t) => t.state === "Matched" && t.departureDay === currentDay);
    // Also handle available travellers departing today (they leave with nothing)
    const emptyDepartures = travellers.filter((t) => t.state === "Available" && t.departureDay === currentDay);
    ...
    for (const traveller of departingTravellers) {
        ...
        const availableVolunteer = volunteers.find((v) => v.state === "Idle" && v.location === "Singapore");
        ...
        const showsUp = (0, distributions_1.sampleBernoulli)(rng, availableVolunteer.reliability);
        if (showsUp) {
            // Volunteer shows up! Handover is successful.
            handleSuccessfulHandover(traveller, requests, availableVolunteer, currentDay, result);
        }
        else {
...
function handleSuccessfulHandover(traveller, requests, volunteer, currentDay, result) {
...
function handleNoVolunteer(traveller, requests, result) {
